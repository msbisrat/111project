...
class BRFSSRiskAdjuster:
    ...
    def _binarize_target(s: pd.Series) -> pd.Series:
        ...
        return s_str.map({"yes": 1, "no": 0, "1": 1, "2": 0, "1.0": 1, "2.0": 0})
    ...
    def fit(self) -> dict:
        ...
        if "Smoked100Cigarettes" in df.columns:
            c = df["Smoked100Cigarettes"].astype(str).str.lower().str.strip() == "yes"
            rr = self._rr(t, c)
            w["ever_smoked_rr"] = round(rr, 4)
        ...
        if "Height" in df.columns and "Weight" in df.columns:
            ...
            w["obese_bmi_rr"] = 1.28  # CDC literature: obesity ~28% higher CVD risk
